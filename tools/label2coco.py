...
class labelme2coco(object):
        ...
        for num,json_file in enumerate(self.labelme_json):
            with open(json_file,'r') as fp:
                ...
                for shapes in data['shapes']:
                    ...
                    h = max(p[:,1])-min(p[:,1])
                    area = w*h
                    self.annotations.append(self.annotation(points,label,area,num))
                    self.annID+=1
    ...
    def annotation(self,points,label,area,num):
        annotation={}
...
DLH101_labelme_root = r'E:\Project_subject\DLH-101\defect detection\DLH_SSD300\data\DLH101\Segmentation\*.json'
labelme_json=glob.glob(DLH101_labelme_root)
# labelme_json=['./1.json']

labelme2coco(labelme_json,'./DLH101_coco.json')