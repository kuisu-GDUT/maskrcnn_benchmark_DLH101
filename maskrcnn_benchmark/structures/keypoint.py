...
def keypoints_to_heat_map(keypoints, rois, heatmap_size):
    #keypoints为真实的keypoints[N,17,3]. 根据roi转换成heatmap
    if rois.numel() == 0:
    ...
    x = (x - offset_x) * scale_x#相对于box左上角的点的偏移位置
    x = x.floor().long()
    y = (y - offset_y) * scale_y
    y = y.floor().long()
    
    x[x_boundary_inds] = heatmap_size - 1#将边界点的值设定为heatmap_size
    y[y_boundary_inds] = heatmap_size - 1

    valid_loc = (x >= 0) & (y >= 0) & (x < heatmap_size) & (y < heatmap_size)#0<keypoints<heatmap_size
    vis = keypoints[..., 2] > 0#可见的keypoints
    valid = (valid_loc & vis).long()#keypoints在roi中, 且大小不超过heatmap_size的点为有效点. s=1

    lin_ind = y * heatmap_size + x#FIXME 关键,y*w+x表示关键点对应的索引位置
