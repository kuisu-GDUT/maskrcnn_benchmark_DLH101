...
def heatmaps_to_keypoints(maps, rois):
    ...
    widths_ceil = np.ceil(widths)# 向上取整
    ...
    for i in range(len(rois)):
        ...
        #torch 双线性插值法
        # torch.nn.functional.interpolate(maps[i],size=(roi_map_width, roi_map_height),mode='bilinear')

        # Bring back to CHW
        roi_map = np.transpose(roi_map, [2, 0, 1])
        # roi_map_probs = scores_to_probs(roi_map.copy())
        w = roi_map.shape[2]
        #todo: 获得关键点, 每个层取出一个关键点
        pos = roi_map.reshape(num_keypoints, -1).argmax(axis=1)#返回沿着axis=1的最大值的索引
        x_int = pos % w#取余数
        y_int = (pos - x_int) // w#去整数
        # assert (roi_map_probs[k, y_int, x_int] ==
        #         roi_map_probs[k, :, :].max())
        x = (x_int + 0.5) * width_correction
        y = (y_int + 0.5) * height_correction
        xy_preds[i, 0, :] = x + offset_x[i]#对应于原图的位置
        xy_preds[i, 1, :] = y + offset_y[i]
        xy_preds[i, 2, :] = 1#1表示为存在该特征点.
        end_scores[i, :] = roi_map[np.arange(num_keypoints), y_int, x_int]#每个点对应的置信度得分
