...
def project_keypoints_to_heatmap(keypoints, proposals, discretization_size):
    #将keypoints根据proposal的box, 转化成对应的heatmap
...
class KeypointRCNNLossComputation(object):
    ...
    def prepare_targets(self, proposals, targets):
        #为每个proposals分配label, 以及其对应的keypoints.
        labels = []
        keypoints = []
        for proposals_per_image, targets_per_image in zip(proposals, targets):
            matched_targets = self.match_targets_to_proposals(
                proposals_per_image, targets_per_image
            )#为每个proposal分配对应target的box对应, label
            ...
            neg_inds = matched_idxs == Matcher.BELOW_LOW_THRESHOLD#获取负样本的索引
            labels_per_image[neg_inds] = 0

            keypoints_per_image = matched_targets.get_field("keypoints")#获取keypoints
            within_box = _within_box(
                keypoints_per_image.keypoints, matched_targets.bbox
            )#获取在box中的关键点
            vis_kp = keypoints_per_image.keypoints[..., 2] > 0#预测为关键点的坐标
            is_visible = (within_box & vis_kp).sum(1) > 0#可见的关键点

            labels_per_image[~is_visible] = -1#非box中的关键点
    ...
    def subsample(self, proposals, targets):
        ...
        labels, keypoints = self.prepare_targets(proposals, targets)#keypoints:  [512, 17,3]
    ...
    def __call__(self, proposals, keypoint_logits):#keypoint_logits:[N,17,56,56]
        heatmaps = []#处理后的keypoints的heatmap
        valid = []#那些keypoints时有效的
        for proposals_per_image in proposals:
            kp = proposals_per_image.get_field("keypoints")#真实的keypoints
            #todo keypoints损失计算时, 数据处理的关键
            heatmaps_per_image, valid_per_image = project_keypoints_to_heatmap(
                kp, proposals_per_image, self.discretization_size
            )#heatmpas_per_image, 每个keypoint的特征图上, 有效的点索引位置
        ...
        keypoint_logits = keypoint_logits.view(N * K, H * W)#特征图的张数, 一张特征图的大小, 一张特征图对应的keypoint,
        #cross_entropy: pred=soft(predict_logits)->pred=log(pred)->loss = -log(pred[target])
        keypoint_loss = F.cross_entropy(keypoint_logits[valid], keypoint_targets[valid])#todo 计算关键点的损失[logits, idx]
