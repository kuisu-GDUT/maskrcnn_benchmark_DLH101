...
class ROIKeypointHead(torch.nn.Module):
    def __init__(self, cfg, in_channels):
        ...
        # self.feature_extractor_sk = make_roi_keypoint_feature_extractor(cfg, in_channels)
    ...
    def forward(self, features, proposals, targets=None):
        ...
        if isinstance(features,torch.Tensor):
            features = [features]
        x = self.feature_extractor(features, proposals)#[72,512,14,14]
        ...
        loss_kp = self.loss_evaluator(proposals, kp_logits)#计算keypoints损失的重要入口[proposal.keypoints-[N,17,3]
