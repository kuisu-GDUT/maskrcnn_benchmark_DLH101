...
class MaskPostProcessor(nn.Module):
    ...
    def forward(self, x, boxes):
        ...
        labels = torch.cat(labels)#有多少个proposal, 就有多少个mask(binary)
        ...
        for prob, box in zip(mask_prob, boxes):
            ...
            for field in box.fields():
                bbox.add_field(field, box.get_field(field))#将box按照软尺寸, 进行处理
            bbox.add_field("mask", prob)
            results.append(bbox)

        return results#输出预测的mask结果
...
def paste_mask_in_image(mask, box, im_h, im_w, thresh=0.5, padding=1):
    ...
    im_mask = torch.zeros((im_h, im_w), dtype=torch.bool)#构建一个跟图像一样大小的im_mask, 值为0
    x_0 = max(box[0], 0)
    ...
    ]#将roi对应的feature, 预测出mask, 再将mask粘贴到原图对应的box
    return im_mask
...
class Masker(object):
    ...
    def forward_single_image(self, masks, boxes):
        ...
        if len(res) > 0:
            ...
            res = masks.new_empty((0, 1, masks.shape[-2], masks.shape[-1]))#没有mask
        return res
